...
document.addEventListener('DOMContentLoaded', function () {
    console.log('[Player] DOMContentLoaded, starting initialization...');
    console.log('[Player] URL params:', window.location.search);
    
    // 先检查用户是否已通过密码验证
    const passwordVerified = isPasswordVerified();
    console.log('[Player] Password verified:', passwordVerified);
    console.log('[Player] Password protected:', isPasswordProtected());
    console.log('[Player] ENV PASSWORD:', window.__ENV__?.PASSWORD ? 'set' : 'not set');
    
    if (!passwordVerified) {
        console.log('[Player] Password not verified, showing modal');
        // 隐藏加载提示
    ...
    console.log('[Player] Password verified, initializing page content');
...
function initializePageContent() {
    console.log('[Player] initializePageContent called');
    
    // 解析URL参数
    const urlParams = new URLSearchParams(window.location.search);
    console.log('[Player] URL params parsed:', Object.fromEntries(urlParams.entries()));
    ...
    console.log('[Player] Current video title:', currentVideoTitle);
    
    // 初始化播放器
    if (videoUrl) {
        console.log('[Player] Initializing player with URL:', videoUrl);
        initPlayer(videoUrl);
    } else {
        console.error('[Player] No video URL provided, showing error');
        showError('无效的视频链接，请返回首页重新选择视频');
    }
